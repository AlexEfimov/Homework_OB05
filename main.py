...
class Player(GameObject):
    def __init__(self, image_path, position, size):
        ...
        self.health = 100  # Начальная стамина
    ...
    def move(self, dx, dy):
        ...
        if 0 <= new_x <= SCREEN_WIDTH - PLAYER_SIZE[0]:
            self.rect.x = new_x
        if EXTRA_HEIGHT <= new_y <= TOTAL_SCREEN_HEIGHT - PLAYER_SIZE[1]:
            self.rect.y = new_y
...
class Header(GameScreen):
    def __init__(self, width, height, bg_image, font_name, font_size, font_color, position, score, health):
        super().__init__(width, height, bg_image, font_name, font_size, font_color, position)
        self.health = health
        self.score = score
    ...
    def draw(self, screen):
        super().draw(screen)
        health_text = self.font.render(f'HEALTH: {self.health}', True, self.font_color)
        score_text = self.font.render(f'SCORE: {self.score}', True, self.font_color)

        # Отображение стамины и очков в верхней панели
        screen.blit(health_text, (10, 10))  # Размещение немного ниже верхней границы
...
class LostScreen(GameScreen):
    ...
    def draw(self, screen):
        ...
        self.score_text = self.font.render(f'SCORE: {self.score}', True, (0, 75, 75))
...
class WinScreen(GameScreen):
    ...
    def draw(self, screen):
        ...
        screen.blit(self.text, self.text_rect)
        self.score_text = self.font.render(f'Счёт: {self.score}', True, (250, 250, 0))
        self.score_text_rect = self.score_text.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2 + 10))
        screen.blit(self.score_text, self.score_text_rect)
        screen.blit(self.control_text, self.control_text_rect)
    def update_score(self, new_score):
...
class Game:
    def __init__(self):
        ...
        self.header = Header(SCREEN_WIDTH, EXTRA_HEIGHT, HEADER_IMAGE, FONT_NAME, FONT_SIZE, HEADER_FNT_COLOR, (0, 0), 0, 0)
        self.door = Door(DOOR_CLOSED, (SCREEN_WIDTH - DOOR_SIZE[0], EXTRA_HEIGHT), DOOR_SIZE, DOOR_OPENED)
        self.player = Player(PLAYER_IMAGE, (0, SCREEN_HEIGHT - PLAYER_SIZE[1] + EXTRA_HEIGHT), PLAYER_SIZE)

        self.lost_screen = LostScreen(SCREEN_WIDTH, SCREEN_HEIGHT, 'None', FONT_NAME, FONT_SIZE, FONT_COLOR, (0, 0), 0)
        self.win_screen = WinScreen(SCREEN_WIDTH, SCREEN_HEIGHT, 'None', FONT_NAME, FONT_SIZE, FONT_COLOR, (0, 0), 0)
        self.sound_manager = SoundManager()
        self.enemies = []
        self.prizes = []
        self.prize_count = 0
    ...
    def restart_game(self):
        ...
        self.victory = False
        self.player.rect.topleft = (0, TOTAL_SCREEN_HEIGHT - PLAYER_SIZE[1])
        self.door.close()
            ...
            if not self.game_over:
                ...
                for enemy in self.enemies:
                    ...
                    if self.player.rect.colliderect(enemy.rect):
                        ...
                        status = self.player.decrease_health(INJURY)    # Уменьшение стамины и проверка на проигрыш
                        if status:
                ...
                self.header.change_health(self.player.health)
                self.header.draw(self.screen)
                ...
                if not self.victory:
                    self.lost_screen.update_score(self.score)
                    self.lost_screen.draw(self.screen)
                else:
                    print(f"Updated score before to win_screen: {self.score}")  # Для отладки
                    self.win_screen.update_score(self.score)
